...
def yolo_detect(image):
    img = image
    boxs = yolo.detect_image(Image.fromarray(image))
    if(len(boxs) == 0):
    ...
    for i in range(len(boxs)):
        left, top, right, bottom = boxs[i]
        if(abs(left - right) * abs(top - bottom) > S_max):
            index = i
    return boxs[index]
...
def predict(image, model):
    ...
    img = img / 255.
    preds1 = model.predict(np.reshape(img, (1, 224, 224, 3)))
    prob1 = np.max(preds1)
    class_id1 = np.argmax(preds1)
    # print("1: {}".format(classes[class_id1]) + "    {}".format(prob1))
    left, top, right, bottom = yolo_detect(image)
    img2 = img2[top:bottom, left:right]
    # cv2.imwrite("test.jpg", img2)
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
    img2 = preprocess.preprocess_image(img2)
    img2 = img2 / 255.
    preds2 = model.predict(np.reshape(img2, (1, 224, 224, 3)))
    prob2 = np.max(preds2)
    class_id2 = np.argmax(preds2)
    # print("2: {}".format(classes[class_id2]) + "    {}".format(prob2))
    preds = (preds2*2 + preds1)/3
    prob = np.max(preds)
    class_id = np.argmax(preds)
    # print("res: {}".format(classes[class_id]) + "    {}".format(prob))
